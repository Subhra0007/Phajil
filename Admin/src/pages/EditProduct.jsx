...
export default function EditProduct() {
  ...
  const [variants, setVariants] = useState([]); // [{ color, images: File[], existingImages: string[], sizes: [{size, stock}], isDefault: boolean }]
    ...
    try {
      ...
      setVariants(
        (p.variants || []).map((v) => ({
          ...
          isDefault: v.isDefault || false, // Default to false if not present
        }))
      );
    } catch (err) {
  ...
  const addVariant = () => {
    setVariants((prev) => [
      ...prev,
      { color: "", images: [], existingImages: [], sizes: [{ size: "", stock: 0 }], isDefault: false },
    ]);
  };
    ...
    try {
      ...
      formData.append(
        ...
        JSON.stringify(
          variants.map((v) => ({
            color: v.color,
            existingImages: v.existingImages, // Keep old images
          ...
          {variants.map((v, vIndex) => (
            <div key={vIndex} className="border rounded-md p-3 mt-3">
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <label className="block text-xs font-medium">Existing Images</label>
                  <div className="flex gap-2 flex-wrap">
                    {(v.existingImages || []).map((url, i) => (
                      <img
                        key={i}
                        src={url}
                        alt="variant"
                ...
                {/* Upload new images */}
                <div className="flex-1">
                  <label className="block text-xs font-medium">Add New Images</label>
                  <input
                    type="file"
                    multiple
